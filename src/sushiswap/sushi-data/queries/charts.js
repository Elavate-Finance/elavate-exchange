const { getWeek, subWeeks, subYears, startOfMinute, getUnixTime } = require('date-fns');
...
module.exports = {
	async factory() {
		...
		data.forEach((entry, i) => {
			...
			weeklyData[startIndexWeekly].weeklyVolumeUSD =
				(weeklyData[startIndexWeekly].weeklyVolumeUSD ? weeklyData[startIndexWeekly].weeklyVolumeUSD : 0) +
				data[i].volumeUSD;
		});
	...
	async tokenHourly({ token_address = undefined, startTime = undefined }) {
		...
		tokenData = tokenData.map((tokenEntry, i) => ({
			...
			untrackedVolumeUSD: tokenData[i - 1]
				? tokenEntry.untrackedVolumeUSD - tokenData[i - 1].untrackedVolumeUSD
				: undefined,
