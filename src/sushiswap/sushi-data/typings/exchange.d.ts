...
export function observeToken({ token_address }: { token_address: string }): {
	subscribe({ next, error, complete }: { next?(data: Token): any; error?(error: any): any; complete?: Function }): any;
};
...
export function observePair({ pair_address }: { pair_address: string }): {
	subscribe({ next, error, complete }: { next?(data: Pair): any; error?(error: any): any; complete?: Function }): any;
};
...
export function ethPrice({ block, timestamp }?: { block?: number; timestamp?: number }): Promise<EthPrice>;

export function ethPriceHourly({
	minTimestamp,
	maxTimestamp,
	minBlock,
	maxBlock,
...
export function factory({ block, timestamp }?: { block?: number; timestamp?: number }): Promise<Factory>;
...
export function twentyFourHourData({
	block,
	timestamp,
}?: {
	block?: number;
